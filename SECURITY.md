...
This policy covers `protowire-swift` — the Swift port of the `protowire`
...
- Issues in `apple/swift-protobuf` itself — file those upstream and
  CC us.
...
on every PR via the `cmd/check-decode` harness. A regression on the
corpus blocks merge.
